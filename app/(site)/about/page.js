...
export default function AboutPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-5 py-16 md:px-8">
      ...
      <h1 className="font-display text-4xl font-bold text-navy">About Mecca City Boutique</h1>

      <div className="mt-8 space-y-5 text-navy-600">
        <p>
      ...
      <div className="tag-divider my-10 text-navy-100" />

      <div className="grid gap-6 rounded-2xl border border-navy-100 bg-white p-8 sm:grid-cols-2">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-electric-600">Find us</p>
          <p className="mt-2 font-display text-lg font-semibold text-navy">{SHOP_LOCATION}</p>
        </div>
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-electric-600">Call or WhatsApp</p>
          {SHOP_PHONES.map((phone) => (
            <p key={phone} className="mt-1 text-navy-600">{phone}</p>
          ))}
          <a
            href={waHref}
            target="_blank"
